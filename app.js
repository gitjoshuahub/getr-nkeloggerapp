...
async function sendAction(params){
  ...
  if(!navigator.onLine){ pushQueue(params); toast("Offline gespeichert – wird später gesendet"); return; }
  const ok = await rawGet(params);
  if(!ok){ pushQueue(params); toast("Fehler – offline gespeichert"); }
}
...
function restoreCachedLager(){
  const cached = localStorage.getItem(LAGER_CACHE_KEY);
  if(cached){
...
async function loadLager(){
  ...
  try{
    const res = await fetchWithTimeout(buildUrl({action:"getlager"}), 8000);
    if(!res.ok){ div.textContent = "Serverfehler (HTTP " + res.status + ")."; return; }
    const data = await res.json();
    const text = "Gesamtbestand: " + data.bestand + " Flaschen";
    div.textContent = text;
    localStorage.setItem(LAGER_CACHE_KEY, text);
  }catch(e){
    const cached = localStorage.getItem(LAGER_CACHE_KEY);
    div.textContent = cached ? cached + "\n\n(offline, zuletzt gespeicherter Stand)" : "Fehler beim Laden.";
  }
}
...
function doEinkauf(){
  ...
  sendAction({ action:"einkauf", menge:kisten, typ:"kasten" });
  toast("Einkauf: " + kisten + " Kisten");
}

function doInventur(){
  const flaschen = document.getElementById("inventurFlaschen").value;
  if(!flaschen){ toast("Bitte Flaschenzahl eingeben"); return; }
  sendAction({ action:"inventur", menge:flaschen });
  toast("Inventur: " + flaschen + " Flaschen");
}
